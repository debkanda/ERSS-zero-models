...
dat.fit=hurdle(count~MCQ010+as.numeric(RIDAGEYR)+RIAGENDR+HID040, data=nhanes, dist="negbin")
...
while (ii <=iter){
  ...
  ###SRS model  
  fitSRS<-hurdle(count~MCQ010+as.numeric(RIDAGEYR)+RIAGENDR+HID040, data=srsdat, dist="negbin")
  mymod0=summary(fitSRS) 
  
  ### ERSS model
  fitRSS<-hurdle(count~MCQ010+as.numeric(RIDAGEYR)+RIAGENDR+HID040, data=rssdat, dist="negbin")
