...
for (p in 1:4) {
  ...
  for (s in 1:3) {
    ...
    RSSdata<-matrix(0,nrow=m*r,ncol=3)
    ...
    while (ii <=iter){
      tryCatch({
        ...
        #Sampling from the mixture to obtain zero-inflated response variable
        y = rpois(n=N,lambda=exp(beta0+betax*x[,1]+betaz*x[,2]))
        
        y1 <- ifelse(U == 0, 0, y)
        SRSdata<-cbind(y1,x)     #this is SRS data
        ...
        y<-rpois(n=N,lambda=exp(beta0+betax*RSSx[,1]+betaz*RSSx[,2]))
        
        y2 <- ifelse(U == 0, 0, y)
        RSSdata<-cbind(y2,RSSx)     #this is ERSS data
        
        
        # Models for SRSdata#
        fitSRS<-zeroinfl(SRSdata[,1]~SRSdata[,2]+SRSdata[,3],dist="poisson")
        mymod0=summary(fitSRS)
        
        # Models for RSSdata#
        fitRSS<-zeroinfl(RSSdata[,1]~RSSdata[,2]+RSSdata[,3],dist="poisson")
