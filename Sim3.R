...
for (p in 1:4) {
  ...
  for (s in 1:3) {
    ...
    covx<-matrix(0, nrow=m*r, ncol=5)
    RSSdata<-matrix(0,nrow=m*r,ncol=8)
    ...
    while (ii <=iter){
      tryCatch({
        ...
        #x[,2]<-x[,2]>0
        c<-mvrnorm(n=r*m,mu=c(0,0,0,0,0), Sigma=sig_c)
        ...
        y = rnbinom(n=r*m, size=1, mu=exp(beta0+betax*x[,1]+betaz*x[,2]+
                                   betac*c[,1]+betac*c[,2]+
                                   betac*c[,3]+betac*c[,4]+
                                   betac*c[,5]))
        ...
        #RSSx[,1]<-RSSx[,1]>0;
        y<-rnbinom(n=r*m, size=1, mu=exp(beta0+betax*RSSx[,1]+betaz*RSSx[,2]+
                                    betac*c[,1]+betac*c[,2]+
                                    betac*c[,3]+betac*c[,4]+
                                    betac*c[,5]))
        y2 <- ifelse(U == 0, 0, y)
